...
genJwt, mustGenJwt
    Generates a JSON Web Token (JWT) with the given claims, algorithm, and base64-encoded key.
    Takes 3 arguments: claims (map[string]any), algorithm (string), and b64Key (string). Claims can
    be created using the Sprig "dict" function.
    Supported algorithms: RS* (RSA), ES* (ECDSA), and HMAC variants (e.g., "RS256", "ES256", "HS256").
    The key must be base64 encoded. For RSA and ECDSA, the key should be a PEM-encoded private key.
    genJwt returns an empty string if there is a problem and mustGenJwt triggers an error in the
