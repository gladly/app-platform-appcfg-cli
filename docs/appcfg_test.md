...
"_test_" sub folder of each configuration. Test output will be written to
the "_test_/_output_" folder when output is directed to a file.
...
relevant template data attributes in the "_test_" folder and "expected_*.*"
validation files in the "_test_/success" dataset folder. Tests are organized
...
Test datasets live in sub folders under the "_test_" folder, where each
...
The template data attribute files in the "_test_" folder serve as defaults
...
```
appcfg test [action | action-header | auth-header | data-pull | data-pull-header | oauth | oauth-header | signing-header | ui-form] [flags]
```
...
* [appcfg test ui-form](appcfg_test_ui-form.md)	 - Test an individual or all UI form configurations
